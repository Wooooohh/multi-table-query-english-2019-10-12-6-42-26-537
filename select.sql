...
WHERE s.id = sc.studentId AND sc.courseId = 1;
  
# 2.Query the presence of both 1 and 2 courses
SELECT * 
FROM student s
	LEFT JOIN student_course sc ON s.id = sc.studentId  AND sc.courseId = 1
	LEFT JOIN student_course sc2 ON s.id = sc2.studentId AND sc2.courseId = 2 
  
# 3.Query the student number and student name and average score of students whose average score is 60 or higher.
select s.id,s.name
from student s
	join (select studentId,avg(score) avg from student_course GROUP BY studentId) as s_asco
	on s.id = s_asco.studentId
where sc.avg >= 60;
